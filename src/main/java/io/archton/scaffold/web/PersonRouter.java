...
public class PersonRouter {
    ...
    public Response createPersonFromForm(@FormParam("firstName") String firstName,
        ...
        try {
            ...
            // Set relationships with validation
            if (titleId != null) {
            ...
            if (genderId != null) {
                ...
                if (person.gender == null) {
                    throw new EntityNotFoundException("Gender", genderId);
                }
            }
            if (idTypeId != null) {
                person.idType = idTypeService.findById(idTypeId);
                if (person.idType == null) {
                    throw new EntityNotFoundException("IdType", idTypeId);
                }
            }
    ...
    public Response updatePersonFromForm(@PathParam("id") Long id,
        ...
        try {
            ...
            String html = Templates.create(titleList, genderList, idTypeList, errorMessage, null).render();
            return WebErrorHandler.createErrorResponse(html, e);
        } catch (Exception e) {
            WebErrorHandler.logError("Error updating person", e);

            // Get the original person record to preserve user input
            Optional<Person> personOpt = personService.findByIdOptional(id);
            if (personOpt.isEmpty()) {
                // This shouldn't happen since the update method would have thrown EntityNotFoundException
